...
import { Carausel } from "../components/carousel_card";
import { useLocation, useNavigate } from "react-router-dom";
import { Channel, Podcast, Comment } from "@/types/types";
...
function ChannelPage() {
  ...
  const { data, channel_subs, userId } = useApiContext();
  ...
  podcasts?.reduce((n, podcast) => {
    arrangedPodcast[podcast.category] = arrangedPodcast[podcast.category] ?? [];

    arrangedPodcast[podcast.category].push(podcast);
  }, arrangedPodcast)




  let defaultImg = channel_data.name.toLowerCase().includes('computing') ? "./images/i5.png" : "./images/i6.png";

  const [getSubsUnsubs, setSubsUnsubs] = useState(false);
  const [isVideoPlaying, setIsVideoPlaying] = useState(false);
  const [selectedVideo, setSelectedVideo] = useState(podcasts[0]);
  const videoRef = useRef(null);

  let subscribed = channel_subs[channel_data.id + '']?.includes(userId);
    ...
    <div className="w-100 h-screen antialiased font-sans bg-gray-100 overflow-hidden overflow-y-auto">
      <div className="container w-100 mx-auto">
        ...
        <section className="relative flex my-6">
          ...
          <div className="flex flex-col justify-start items-start p-6 min-w-96 bg-gray-200 ml-6 rounded-sm overflow-hidden">
            ...
            <div className="flex justify-between gap-0 mt-3 w-full">
              ...
              <button
                type="button"
                className={`font-bold text-xs rounded-sm py-2 px-6 ${subscribed === true
        ...
        <section>
          ...
          {selectedVideo.comments?.map((comment, index) => (<CommentBox key={index} comment={comment} />))}
        ...
        <section className="relative flex flex-col rounded-sm bg-white py-6 px-8 my-6 shadow-lg">
          ...
          <Tab.Group>
            <Tab.List className="flex justify-center items-center border-b border-gray-600">
              ...
              {Object.keys(arrangedPodcast).map((category,index) => (
                <Tab
                key={index}
                  className={({ selected }) =>
                    classNames(
            ...
            <Tab.Panels as="div" className="mt-2">
              ...
              {Object.keys(arrangedPodcast).map(category => (
                <Tab.Panel
                  className={classNames(
                    "bg-white p-3",
                    "focus:outline-none focus:ring-0"
